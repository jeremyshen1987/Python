...
# transaction_table = soup.find('table', class_ = 'chakra-table css-5605sr')

# for tb in transaction_table.find_all('tbody'):
#     rows = tb.find_all('tr')
#     for row in rows:
#         trans_data = row.find('td',class_ = 'css-xumdn4').text
#         print(trans_data)
...
# session = HTMLSession()
# request = session.get('https://www.honestdoor.com/recently-sold/ab/calgary?page=1')
